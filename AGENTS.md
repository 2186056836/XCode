...
- 组件通过 `packages/ui/src/hooks/` 访问服务；平台操作通过 `IPlatformService`（`packages/shared/src/platform.ts`），不直接调用 `window.xcode`。
...
- UI 使用 `packages/ui/src/logger.ts`，不直接使用 `console.log` 或 `window.xcode?.log`。
