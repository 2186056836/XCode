...
export function resolveProductionRemoteAssetCacheDir(
  ...
  if (platform === "darwin") {
    return pathApi.join(homeDir, "Library", "Application Support", "XCode", "remote-assets-cache");
  }

  if (platform === "win32") {
    const appDataDir = env.APPDATA?.trim() || pathApi.join(homeDir, "AppData", "Roaming");
    return pathApi.join(appDataDir, "XCode", "remote-assets-cache");
  }

  const configDir = env.XDG_CONFIG_HOME?.trim() || pathApi.join(homeDir, ".config");
  return pathApi.join(configDir, "XCode", "remote-assets-cache");
}
