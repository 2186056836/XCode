...
import { sanitizeXCodeRuntimeEnv } from "@zcode/shared";
...
export function buildMcpStdioEnv(options: {
  ...
  return prependRunningNodeDirectory(
    applyNetworkEgressEnv(sanitizeXCodeRuntimeEnv(filterStringEnv(sourceEnv)), {
...
function prependRunningNodeDirectory(
  ...
  // remote Agent 由 ~/.xcode/server/node 启动，但登录环境 PATH 不含该目录，
