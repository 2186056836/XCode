...
export async function resolveDefaultSkillRoots(
  ...
  const includeXcode = options.includeXcodeSkills ?? true;
...
function skillRootsForBase(
  ...
  // 合并而不是 fallback：用户可能同时安装原生 `.xcode` skill 和兼容 `.agents` skill。
  // 同一级别仍保持 `.xcode` 优先，后续同名按 root 顺序解析。
  return [
