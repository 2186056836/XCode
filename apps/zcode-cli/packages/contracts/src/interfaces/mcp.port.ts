...
/**
 * XCode 官方 Server MCP 的鉴权声明。
 * 允许出现在 `type: "http"` 与 `type: "stdio"`；`sse` 仍拒。
 * `type`/`provider` 均为精确值，不接受别名或大小写变体。
 *
 * 注意：该字段本身**不构成**官方身份证明。官方身份由运行时解析的 XCode API origin 判定
 * （host 侧还会二次校验）；第三方 Plugin 复制该字段只能让凭证流向真实 XCode 后端。
 *
 * 两种形态的凭证投递通道不同：
 * - http：宿主在 fetch wrapper 里逐请求注入身份头，凭证从不进入插件进程；
 * - stdio：请求由插件进程自己发出，身份头随每条出站协议消息的 `_meta` 下发。
 */
export interface XCodeOfficialMcpAuthConfig {
  type: "zcode_official";
  provider: "jwt_token";
}
...
export interface McpStdioServerConfig extends McpServerConfigBase {
  ...
  auth?: XCodeOfficialMcpAuthConfig;
  /** 宿主生成，禁止来自文件配置。 */
...
export interface McpToolDescriptor {
   ...
   * 而不是"插件是谁"：
   * - http：响应来自 XCode 后端。连接存活即意味着每个请求都过了 origin 校验且 fail closed，
   *   第三方插件即使声明官方鉴权，也只能把请求打到真实 XCode，响应体不由它写；
 ...
 * 官方 MCP 信任判定。
 *
 * 规则只有一条：**目标 origin 逐字符等于当前 XCode API origin（https、无 username/password）**，
