...
export const SessionEventType = {
  ...
  // XCode fork：助手消息文本被原地编辑（保存后作为真实上下文进入后续请求）。
  AssistantMessageEdited: "assistant_message_edited",
...
/** XCode fork：助手消息原地编辑事件的载荷（entityId = transcript assistant messageId）。 */
export interface AssistantMessageEditedPayload {
  entityId: string;
  messageId: string;
  text: string;
}
