...
] as const satisfies readonly XCodeSlashCommand[];
...
function normalizeXCodeSlashCommandName(name: string): string {
  return name.trim().replace(/^\/+/, "").toLowerCase();
}

export function isReservedXCodeSlashCommandName(name: string): boolean {
  return RESERVED_SLASH_COMMAND_NAMES.has(normalizeXCodeSlashCommandName(name));
}
