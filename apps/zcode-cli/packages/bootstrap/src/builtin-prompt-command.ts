...
interface ResolveXCodeBuiltinPromptCommandOptions {
  workingDirectory?: string;
}

export function resolveXCodeBuiltinPromptCommand(
  input: string,
  options: ResolveXCodeBuiltinPromptCommandOptions = {},
): string | undefined {
...
function buildInitAgentsPrompt(params: {
  ...
  return [
    ...
    `- Existing hidden instruction candidates: ${join(params.workingDirectory, ".xcode", "AGENTS.md")} and ${join(params.workingDirectory, ".agents", "AGENTS.md")}`,
    "- File name must be exactly AGENTS.md.",
    "- This command targets the current workspace only. Do not write ~/.xcode/AGENTS.md.",
    additionalInstructions,
    "",
    "Process:",
    "1. First check whether .xcode/AGENTS.md or .agents/AGENTS.md exists in the workspace. If either exists, tell the user they already have an instructions file, mention the path found, and stop without creating a new AGENTS.md.",
