...
import {
  ...
  isXCodeFileStreamingToolInputPreviewTool,
  parseXCodeBackgroundTaskNotificationText,
  resolveXCodeBackgroundTaskControlKind,
...
export class ProductProjection {
  ...
  resolveRowActionTarget(
    ...
    // XCode fork：助手消息原地编辑——与 feedback 同门槛（任意 complete assistant 行），
    // canEditAssistant 由 materializeCommandRowActions 按 state=complete 下发。
    if (action === "editAssistantMessage") {
      const messageId = this.messageIdByRowId.get(row.rowId);
      if (row.kind !== "assistantText" || row.actions?.canEditAssistant !== true || !messageId) {
        return {
          ok: false,
          status: "rejected",
          reasonCode: "guard.actionUnavailable",
  ...
  private applyEventInternal(
    ...
    const reduced =
      event.type === SessionEventType.AssistantFeedbackUpdated
        ...
        : event.type === SessionEventType.AssistantMessageEdited
          ? this.onAssistantMessageEdited(event)
          : (() => {
  ...
  private materializeCommandRowActions(reduced: ConversationDelta[]): ConversationDelta[] {
    ...
    for (const row of rows) {
      ...
      if (row.kind === "turnHeader") {
        ...
        // XCode fork：任意 complete assistant 行可原地编辑文本（不要求轮尾）。
        const canEditAssistant =
          !compactActive && row.state === "complete" && this.messageIdByRowId.has(row.rowId);
        if (canEditAssistant) nextActions.canEditAssistant = true;
        else delete nextActions.canEditAssistant;
      }
  ...
  private applyBackgroundTaskNotification(fact: CanonicalUserIntentFact): ConversationDelta[] {
    const parsed = parseXCodeBackgroundTaskNotificationText(fact.input);
    if (!parsed) return [];
  ...
  /** XCode fork：助手消息原地编辑——按 entityId 找 assistantText 行，整行替换为新文本。 */
  private onAssistantMessageEdited(event: SessionEvent): ConversationDelta[] {
    const payload = event.payload as AssistantMessageEditedPayload;
    const row = this.snapshot.rows.window.find(
      (candidate): candidate is AssistantTextRow =>
        candidate.kind === "assistantText" && candidate.entityId === payload.entityId,
    );
    if (!row) return [];
    if (row.text === payload.text) return [];
    return [{ op: "row.upserted", row: { ...row, text: payload.text } }];
  }
  ...
  private openToolRow(
    ...
    if (isXCodeFileStreamingToolInputPreviewTool(row.toolName)) {
  ...
  private onBackgroundTaskLifecycle(event: SessionEvent): ConversationDelta[] {
    ...
    const legacyKind = resolveXCodeBackgroundTaskControlKind(payload);
