...
export interface OfficialPluginDefinition {
  ...
  // 这样用户首次 `/skill <name>` 就能用,不必先 `xcode plugins enable`。
...
export const OFFICIAL_PLUGIN_DEFINITIONS: readonly OfficialPluginDefinition[] = [
  ...
  },
  {
    ...
    // 让用户/agent 开箱即用地拿到 XCode 配置指南、自诊断技能与 dynamic workflow 编写指南。
    defaultEnabled: true,
    listing: {
      author: ZAI_AUTHOR,
      category: "utilities",
  ...
  },
  {
    ...
    // isXCodeCuaInternalFeatureEnabled（打包层默认 true）与输入框入口 hidden 默认值的联动语义。
    name: "computer-use",
    hostMcpServerNames: ["node_repl"],
...
// 注意: 任何解析 plugin 的入口 (CLI 子命令 resolveXCodePlugins、应用启动 resolveStartupPlugins)
