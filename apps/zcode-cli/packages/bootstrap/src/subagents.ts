...
interface LoadXCodeAgentProfilesInput {
  logger?: Logger;
  storageRoot: string;
  workingDirectory: string;
}

interface LoadXCodeAgentProfilesResult {
...
export async function loadXCodeAgentProfiles(
  input: LoadXCodeAgentProfilesInput,
): Promise<LoadXCodeAgentProfilesResult> {
  ...
  const roots = [
    ...
    { path: join(input.workingDirectory, ".xcode", "agents"), source: "project" as const },
  ];
...
function sanitizeProjectAgentProfile(profile: AgentProfile): AgentProfile {
  ...
  // 项目级 .xcode/agents/*.md 是仓库内容，不能通过 frontmatter
