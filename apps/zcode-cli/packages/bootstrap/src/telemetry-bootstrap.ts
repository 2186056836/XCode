import { getCapturedXCodeAgentTelemetryEnv } from "@zcode/shared";
import {
 ...
 */
export async function prepareXCodeTelemetryEnv(
...
/**
 * 与 prepareXCodeTelemetryEnv 对称地关闭当前进程持有的 Telemetry Owner。
 * 单个 App/Session 只允许 flush；只有最外层可执行入口可以调用本函数。
 */
export async function shutdownXCodeTelemetry(): Promise<void> {
