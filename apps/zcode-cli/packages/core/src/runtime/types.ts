...
import type { XCodeProviderAccountAccess } from "@zcode/shared";
...
import type { PresentationSurface, SystemPromptOverrides } from "../context/types.js";
...
export interface AgentRuntimeConfig {
  ...
  runtimeFeatures?: {
     ...
     * 由 bootstrap 根据 XCode 官方插件启停推导，不由普通插件 manifest 自声明。
     */
    nodeRepl?: boolean;
    /**
  ...
  /**
   * 动态工作流灰度门：Host 判定后经
   * XCode Protocol 下发，runtime 只消费。**缺席即开启**——TUI、headless `-p` 与
  ...
  systemPrompt?: string;
  /**
   * XCode fork：设置页自定义的 system 消息替换内容（仅第 1 条产品身份消息），
   * 会话物化时由 Host 下发冻结。与 systemPrompt（workflow child 整段替换语义）
   * 互不干扰；子代理路径不消费。
   */
...
export interface ProviderRuntimeHeadersPort {
  ...
  refreshBeforeModelRequest(input: {
    accountAccess?: XCodeProviderAccountAccess;
    abortSignal?: AbortSignal;
    modelId: string;
    providerId: string;
