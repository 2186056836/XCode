...
// 品牌类型（SessionId）和 createXCodeApp 的联合签名收窄错。
export async function prepareTuiAppRuntime(
  ...
  const createAppFactory = deps.createXCodeApp ?? bootstrapModule?.createXCodeApp;
  ...
  if (prepareTelemetry) {
    state.shutdownTelemetry =
      deps.shutdownXCodeTelemetry ?? bootstrapModule?.shutdownXCodeTelemetry;
  }
