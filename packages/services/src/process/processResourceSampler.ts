...
export interface HostResourceUsageAgent {
  ...
  children: readonly XCodeProcessChildProcess[];
}
...
type Owner =
  ...
  | { kind: "mcp"; child: XCodeProcessChildProcess }
...
function ownerToRow(
  ...
  if (owner.kind === "agent") {
    return {
      pid: sample.pid,
      name: isOwnerRoot
        ? formatXCodeAgentProcessName(owner.agent.provider, owner.agent.workspacePath)
