...
class XCodeV4CommandRejectedError extends Error {
  ...
  constructor(
    ...
    this.name = "XCodeV4CommandRejectedError";
  }
}
...
export function assertV4CommandAckOk(
  ...
  throw new XCodeV4CommandRejectedError(commandType, ack, contextMessage);
}
...
export async function sendHostCasCommandV4<T extends CommandType>(input: {
  ...
  throw new XCodeV4CommandRejectedError(
    input.type,
    lastAck ?? {
      commandId: "",
      status: "stale",
