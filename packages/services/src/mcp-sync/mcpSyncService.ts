...
const ZCODE_MCP_DESCRIPTOR: DirectoryMcpDescriptor = {
  ...
  userConfigDirSegments: [".xcode", "cli"],
  workspaceConfigDirSegments: [".xcode"],
...
export function createMcpSyncService(
  ...
  return {
    ...
    async checkRemoteUserMcpWriteAccess() {
      return checkRemoteSyncDirectoryWriteAccess(dirname(getUserXcodeMcpConfigPath()));
    },
...
function getUserXcodeMcpConfigPath(): string {
...
async function saveMcpToUserDirectory(payload: SaveCliMcpToUserDirectoryRequest): Promise<void> {
  ...
  await writeXCodeServersToFile(scope, nextServers, payload.projectPath);
}
...
async function writeXCodeServersToFile(
  scope: Exclude<McpScope, "common">,
...
async function readUserCliConfig(): Promise<Record<string, unknown>> {
  return (await readJsonObject(getUserXcodeMcpConfigPath())) ?? {};
}

async function writeUserCliConfig(config: Record<string, unknown>): Promise<void> {
  await writeTextAtomic(getUserXcodeMcpConfigPath(), `${JSON.stringify(config, null, 2)}\n`);
}
...
async function importMcpServers(params: {
  ...
  const targetPath = getUserXcodeMcpConfigPath();
