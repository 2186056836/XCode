...
import { resolveBigModelStartPlanXcodeJwt } from "./bigmodelStartPlanXcodeJwt.js";
import {
...
async function resolveStartPlanAuthorization(
  ...
  if (provider.family === "bigmodel") {
    const zcodeJwtToken = await resolveBigModelStartPlanXcodeJwt({
  ...
  const credentialJwt = await loadZaiProviderConnectionXcodeJwtToken(context);
...
async function loadZaiProviderConnectionXcodeJwtToken(
  context: CodingPlanAvailabilityContext,
): Promise<string> {
...
function hasActiveStartPlan(plans: ZaiStartPlanPlan[] | undefined): boolean {
  return Boolean(
    plans?.some((plan) => {
      ...
      // 和 `name=XCode V3 Start Plan`；只认 `name === "start plan"` 的话，
