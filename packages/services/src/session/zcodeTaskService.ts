/* eslint-disable max-lines -- XCode task wrapper service 接口集中承载 app/runtime API，拆散会让替换阶段更难追踪。 */
...
import type {
  ...
  XCodeTaskCreateResult,
  XCodeTaskMeta,
  XCodeStreamEvent,
  XCodeTaskMode,
  XCodeConfigOption,
  XCodeError,
  XCodeAssistantMessageFeedback,
  XCodePromptAttachment,
  XCodeProvider,
  XCodeWorkspaceEvent,
  TraceId,
  XCodeSessionFile,
  XCodeAgentMcpServer,
  XCodeTaskSnapshot,
  XCodeTaskSnapshotBody,
  XCodeTaskSnapshotToolCallsSlice,
  XCodeTaskSnapshotRefContent,
  XCodeEnqueueTaskCommandResult,
  XCodeCancelTaskCommandResult,
  XCodeTaskClientMode,
  XCodeTaskTokenUsageResult,
...
import type {
  ...
  XCodeTaskListResult,
  XCodeTaskListSortBy,
  XCodeTaskListWorkspaceScope,
  XCodeWorkspaceEventSubscriptionParams,
  XCodeGroupedTaskView,
  XCodeGroupedTaskViewOrderInput,
  XCodeGroupedTaskViewQuery,
  XCodeGroupedTaskViewStructure,
  XCodeTaskGroup,
  XCodeTaskGroupColor,
} from "#src/session/zcodeTaskListTypes.js";

export interface XCodeTaskSnapshotWithEtagResult {
  snapshot: XCodeTaskSnapshot | null;
  etag?: string;
  notModified?: boolean;
}

/** 同一 workspace 的固定归档删除集合；每个去重后的目标恰好属于一种结果。 */
export interface XCodeArchivedTaskDeletionResult {
...
export type XCodeModelTrajectoryContentPart =
...
export interface XCodeModelTrajectoryMessage {
  role: string;
  parts: XCodeModelTrajectoryContentPart[];
}

/** model_io 记录里归一化出来的 token 用量。 */
export interface XCodeModelTrajectoryUsage {
  inputTokens?: number;
  outputTokens?: number;
  totalTokens?: number;
...
export type XCodeModelTrajectoryCallSourceKind =
  | "main"
  | "sidecar"
  | "subagent"
  | "compact"
...
export interface XCodeModelTrajectoryCallSource {
  kind: XCodeModelTrajectoryCallSourceKind;
  querySource?: string;
}
 ...
 */
export interface XCodeModelTrajectoryRecord {
  requestId: string;
  attempt: number;
  startedAt: string;
  completedAt?: string;
  ...
  turnId?: string;
  traceId?: string;
  callSource?: XCodeModelTrajectoryCallSource;
  model: {
    modelId?: string;
    providerId?: string;
    role?: string;
  ...
  request: {
    messages: XCodeModelTrajectoryMessage[];
    toolNames: string[];
  };
  response?: {
    finishReason?: string;
    ...
    toolCalls: XCodeModelTrajectoryContentPart[];
    usage?: XCodeModelTrajectoryUsage;
    responseId?: string;
    modelId?: string;
  };
  error?: {
...
export interface XCodeModelTrajectory {
  taskId: string;
  /** runtime 是否支持读取 model-io 轨迹（仅 XCode Agent 落盘 model-io）。 */
  available: boolean;
  records: XCodeModelTrajectoryRecord[];
  /** 命中的源文件绝对路径，便于排查。 */
  sourceFiles: string[];
...
export type {
  ...
  XCodeGroupedTaskRef,
  XCodeGroupedTaskView,
  XCodeGroupedTaskViewNode,
  XCodeGroupedTaskViewOrderInput,
  XCodeGroupedTaskViewQuery,
  XCodeGroupedTaskViewStructure,
  XCodeGroupedTaskViewStructureMember,
  XCodeGroupedTaskViewStructureTopOrder,
  XCodeGroupedTaskViewTopLevelNodeRef,
  XCodeTaskGroup,
  XCodeTaskGroupColor,
} from "#src/session/zcodeTaskListTypes.js";

/** 一个 task 输入轮次的终态结果，供后台派发（定时任务）回写运行结果。 */
export interface XCodeTaskTerminalOutcome {
  taskId: string;
...
export interface XCodeTaskReadyOutcome {
  taskId: string;
  reason: "prompt_completed" | "prompt_failed";
}

/**
 * IXCodeTaskService — XCode task wrapper API 服务接口
 *
 * UI、remote controller 通过这层访问 task wrapper 状态；核心 session 状态由
 * XCode Agent server 维护，新功能应优先走 IXCodeSessionService。
 */
export interface IXCodeTaskService {
  // ---- 生命周期 ----
  ...
  enqueueTaskCommand(params: {
    ...
    automationId?: string;
    ownerRunId?: TraceId;
  }): Promise<XCodeEnqueueTaskCommandResult>;
  ...
  cancelTaskCommand(params: {
    ...
    ownerRunId?: TraceId;
    clientMode: XCodeTaskClientMode;
  }): Promise<XCodeCancelTaskCommandResult>;
  ...
  }): Promise<XCodeSessionCompactResult>;
  ...
  goalSession(params: {
    ...
    objective?: string;
    expectedRevision?: number;
  }): Promise<XCodeSessionGoalResult>;
  ...
  listGroupedTaskViewStructure(params: {
    workspaceScopes: XCodeTaskListWorkspaceScope[];
  }): Promise<XCodeGroupedTaskViewStructure>;

  /** 一次性提交 grouped 视图最终排序和 membership，服务层用 sqlite transaction 落库 */
  applyGroupedTaskViewOrder(params: XCodeGroupedTaskViewOrderInput): Promise<XCodeGroupedTaskView>;
  ...
  getTaskSnapshot(params: {
    ...
    model?: string;
    thoughtLevel?: string;
  }): Promise<XCodeTaskSnapshot | null>;
  ...
  getTaskSnapshotWithEtag(params: {
    ...
    model?: string;
    thoughtLevel?: string;
  }): Promise<XCodeTaskSnapshotWithEtagResult>;
  ...
  getTaskSnapshotBody(params: {
    ...
    workspaceIdentity?: string;
    refId: string;
  }): Promise<XCodeTaskSnapshotBody | null>;
  ...
  getTaskSnapshotRef(params: {
    ...
    workspaceIdentity?: string;
    refId: string;
  }): Promise<XCodeTaskSnapshotRefContent | null>;
  ...
  getTaskSnapshotToolCallsSlice(params: {
    ...
    limit: number;
  }): Promise<XCodeTaskSnapshotToolCallsSlice | null>;
  ...
  getTaskMeta(params: {
    ...
    workspacePath: string;
    workspaceIdentity?: string;
  }): Promise<XCodeTaskMeta | null>;

  /** 读取当前 active task 内存态的配置选项，不做 workspace 预热兜底 */
  getTaskConfigOptions(params: { taskId: string }): Promise<XCodeConfigOption[]>;
  ...
  setAssistantMessageFeedback(params: {
    ...
    workspaceIdentity?: string;
    turnIndex: number;
    feedback: XCodeAssistantMessageFeedback | null;
  }): Promise<XCodeSessionFile>;
  ...
  /** 切换 task 模式 */
  setMode(params: { taskId: string; mode: XCodeTaskMode }): Promise<void>;
  ...
  /**
   * 读取 task 对应的模型调用轨迹（来自 ~/.xcode/cli/{debug,rollout} 的 model-io JSONL）。
   * taskId 即 XCode Agent 的 sessionId，按 sessionId 匹配 model-io 记录。
   */
  getModelTrajectory(params: {
    taskId: string;
    /** 最多返回的调用条数（按时间倒序保留最近 N 条），默认 200。 */
    limit?: number;
  }): Promise<XCodeModelTrajectory>;
  ...
  getTaskTokenUsage(params: {
    ...
    workspacePath: string;
    workspaceIdentity?: string;
  }): Promise<XCodeTaskTokenUsageResult>;
  ...
  /**
   * 重启指定 workspace 的 XCode Agent 共享进程。
   * 配置变更需要重新读取进程环境时使用。
  ...
  deleteArchivedTasks(params: {
    ...
    workspaceIdentity?: string;
    taskIds: string[];
  }): Promise<XCodeArchivedTaskDeletionResult>;
  ...
  onDynamicWorkspaceEvent(
    workspace: string | XCodeWorkspaceEventSubscriptionParams,
  ): Event<XCodeWorkspaceEvent>;

  /** 全局错误事件 */
  onError: Event<XCodeError>;
}

export const IXCodeTaskService = createServiceDescriptor<IXCodeTaskService>(
  ServiceChannels.XCodeTask,
);
