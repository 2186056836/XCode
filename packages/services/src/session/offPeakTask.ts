...
export interface IOffPeakTaskService {
  ...
  createTask(params: XCodeOffPeakTaskCreateParams): Promise<OffPeakTaskCreateResult>;
  cancelTask(offPeakTaskId: string): Promise<XCodeOffPeakTask | null>;
  pauseTask(offPeakTaskId: string): Promise<XCodeOffPeakTask | null>;
  continueTask(offPeakTaskId: string): Promise<XCodeOffPeakTask | null>;
  deleteTask(offPeakTaskId: string): Promise<void>;
  /** 仅隐藏本地 History 行；不删除 task/session/执行字段。 */
  deleteHistory(offPeakTaskId: string): Promise<XCodeOffPeakTask | null>;
  updateTask(
    offPeakTaskId: string,
    params: OffPeakUpdateTaskParams,
  ): Promise<XCodeOffPeakTask | null>;
  list(): Promise<XCodeOffPeakTask[]>;
  get(offPeakTaskId: string): Promise<XCodeOffPeakTask | null>;
}
