...
import type { XCodeSessionFile, XCodeTaskMeta } from "@zcode/shared";
...
export function buildSearchableTextFromMessages(messages: XCodeSessionFile["messages"]): string {
  const parts: string[] = [];
  let total = 0;
...
export async function writeImportedClaudeTaskSnapshot(params: {
  ...
  // ~/.xcode/v2/sessions/{hash}/{taskId}.json。现在真实 XCode session 承担续聊，legacy snapshot
