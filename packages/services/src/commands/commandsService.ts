...
const ZCODE_COMMAND_DESCRIPTOR: CommandAgentSourceDescriptor = {
  ...
  userDirectorySegments: [".xcode", "commands"],
  workspaceDirectorySegments: [".xcode", "commands"],
  fileExtension: ".md",
  format: "markdown",
  namespaceSeparator: "/",
...
export function createCommandsService(_options?: CommandsServiceOptions): ICommandsService {
  async function list(params: {
    ...
    // XCode Agent 需要先合并所有 workspace 目录，再合并所有 user 目录；
    // 按每个目录交错读取 project/user 会让 user .xcode 抢在 workspace .agents 前面。
    ...
    return {
      commands: [...dedupedUserCommands, ...pluginCommands] as XCodeCommand[],
...
async function discoverCommandsFromDirectorySources(params: {
  ...
  for (const descriptor of params.descriptors) {
    ...
    // `.xcode` 是强优先级来源；只要读到有效命令，同 scope 的 `.agents` 就不再参与。
