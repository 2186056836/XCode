...
/**
 * 把 host 已解析出的**权威 XCode API origin** 下发给 agent 子进程。
 *
 * 两侧的官方 MCP 信任判定共用 `@zcode/shared` 的同一份实现，但**输入**
 * 曾经分叉——host 用 `resolveRuntimeXCodeEndpointOrigin(env, { overrideOrigin: settings
 * .zcodeEndpointOrigin })`，agent 只有 `resolveRuntimeXCodeEndpointOrigin(env)`，而 settings
 ...
 * `resolveRuntimeXCodeEndpointOrigin`（信任判定、provider 路由来源头、model-config、
...
export function buildAgentEndpointOriginEnv(
  ...
  // ZCODE_BASE_URL 是 resolveRuntimeXCodeEndpointOrigin 读取 envBaseOrigin 的最高优先级键，
