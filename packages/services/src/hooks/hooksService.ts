...
function getRootDir(source: SettingsDirectorySource, workspacePath?: string): string {
  ...
  if (source === "zcode") {
    return workspacePath ? join(baseDir, ".xcode") : join(baseDir, ".xcode", "cli");
  }
...
async function readUserXCodeSource(): Promise<WorkspaceHookSourceInput | undefined> {
...
async function writeXCodeHooksConfig(
  workspacePath: string | undefined,
  hooks: Hook[],
): Promise<void> {
  const configPath = getConfigPath("zcode", workspacePath);
  const existingConfig = (await readJsonFile<XCodeConfigFile>(configPath)) ?? {};
  ...
  await atomicWriteWorkspaceHookConfig(configPath, {
    ...
    hooks: {
      ...
      events: toXCodeHooksEvents(hooks),
    },
  });
}
...
async function saveHooksImpl(params: {
  ...
  const currentProjectConfigPath = resolve(params.workspacePath, ".xcode", "config.json");
  ...
  await writeXCodeHooksConfig(undefined, userHooks);
  await writeXCodeHooksConfig(params.workspacePath, projectHooks);
}
