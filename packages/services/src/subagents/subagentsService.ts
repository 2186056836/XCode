...
async function discoverPluginAgents(params: {
  ...
  const storageRoot = await resolveXCodeStorageRoot(params.storageOptions);
...
async function readPluginConfig(options?: SubagentStorageOptions): Promise<PluginConfigSummary> {
  try {
    const configPath = join(resolveUserHomeDir(options), ".xcode", "cli", "config.json");
...
export function createSubagentsService(options?: SubagentsServiceOptions): ISubagentsService & {
  ...
  return {
    ...
    async getPrimaryUserAgentsDirectory(_params: { provider: XCodeProvider }): Promise<{
      path: string;
    }> {
