...
const WINDOWS_INSTALL_MANIFEST_NAME = ".xcode-install-manifest";
...
export default {
  ...
  extraResources: [
    ...
    {
      // Provider Registry 的 XCode Built-in Config 是静态 Provider/Model 事实的唯一内置来源。
      // 显式随包发布，避免正式 Host 回退到旧 Catalog/Preset hardcode。
      // XCode fork：改用相对 app 目录的路径——实测绝对路径 from 在 Windows 打包时
      // 不会覆盖 resources 里的旧文件（内置配置滞留在旧 revision），相对路径由
      // electron-builder 自行解析，覆盖行为正常。
      from: "../../config/provider/zcode-builtin.json",
  ...
  linux: {
    ...
    // 与 /usr/share/icons/hicolor/*/apps/xcode.png 保持一致。
