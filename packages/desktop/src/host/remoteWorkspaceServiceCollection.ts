...
export function createRemoteWorkspaceServiceCollection(params: {
  ...
  createReportingRemoteXCodeTaskService: <T extends object>(service: T) => T;
  ...
  const reportingRemoteXCodeTaskService = params.createReportingRemoteXCodeTaskService(
  ...
  const remoteXCodeTaskService = params.createRemotePromptAttachmentTaskService(
    reportingRemoteXCodeTaskService,
  );
  const remoteXCodeSessionService = params.createRemotePromptAttachmentSessionService(
  ...
  // 因此这里为 remote workspace host 补齐本地全局 channel；文件、终端、XCode Agent 仍来自远端，
  ...
  const services = new ServiceCollection()
    ...
    .register(IXCodeTaskService, remoteXCodeTaskService)
    .register(IXCodeAgentService, params.connectionServices.zcodeAgentService)
    .register(IXCodeSessionService, remoteXCodeSessionService)
