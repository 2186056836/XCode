...
import type { TraceId, XCodePromptAttachment } from "@zcode/shared";
import { randomUUID } from "node:crypto";

const REMOTE_PROMPT_ATTACHMENT_ROOT = "~/.xcode/tmp/prompt-attachments";
const REMOTE_PROMPT_ATTACHMENT_RELATIVE_ROOT = ".xcode/tmp/prompt-attachments";
...
export async function materializeRemotePromptAttachments(
  ...
  const nextAttachments: XCodePromptAttachment[] = [];
...
function getAttachmentLocalPath(attachment: XCodePromptAttachment): string | undefined {
      ...
      return async (...args: unknown[]) => {
        ...
        const prepared = await options.materializePromptAttachments({
          ...
          attachments: Array.isArray((params as { attachments?: unknown }).attachments)
            ? (params as { attachments?: XCodePromptAttachment[] }).attachments
            : undefined,
        });
