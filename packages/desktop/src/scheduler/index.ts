...
function resolveScheduledAt(automation: XCodeAutomation, now: number): number {
...
async function handleClaimed(automation: XCodeAutomation, now: number): Promise<void> {
...
function postDispatchRequest(
  ...
  runId: string,
  fixedSelection?: XCodeAutomationRun["modelSelection"],
): void {
 ...
 */
async function handleOffPeakClaimed(task: XCodeOffPeakTask, now: number): Promise<void> {
...
async function settleDispatchResult(
  ...
  const trigger: XCodeAutomationTrigger =
