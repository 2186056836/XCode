...
function resolveForceUpdateClientConfigUrl(endpointOrigin = DEFAULT_ZCODE_ENDPOINT_ORIGIN): string {
  const url = new URL(
    `${buildXCodeEndpointUrls(endpointOrigin).origin}${ZCODE_CLIENT_CONFIG_API_PATH}`,
  );
...
function getForceUpdateMinimalVersionFromClientConfig(config: unknown): string | undefined {
  ...
  if (typeof envelope.code === "number" && envelope.code !== 0) {
    ...
    throw new Error(`XCode client config failed: ${envelope.code}`);
  }
...
export async function maybeBlockStartupForForceUpdate(
  ...
  // XCode fork：禁用更新检测——强更门不拉远端 manifest、不阻止启动、不弹强更对话框。
  const XCODE_FORK_AUTO_UPDATES_DISABLED = true;
  if (XCODE_FORK_AUTO_UPDATES_DISABLED) {
