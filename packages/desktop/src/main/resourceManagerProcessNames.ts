...
import { formatXCodeRendererProcessName } from "@zcode/shared";
...
export function buildAuxiliaryRendererName(contents: WebContents): string {
  ...
  if (url?.startsWith("devtools://")) {
    return formatXCodeRendererProcessName(
      `devtools-${contentsId}-${hostTitle ?? ownerLabel ?? "unknown"}`,
    );
  }

  if (type === "window") {
    return formatXCodeRendererProcessName(title ?? `window-${contentsId}`);
  }

  return formatXCodeRendererProcessName(`${type}-${contentsId}-${pageLabel}`);
}
