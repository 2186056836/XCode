...
export type XCodeDataSizePartialReason = "file_limit" | "io_error" | "time_limit";

export type XCodeDataSizeScanResult = {
  bytes: number;
  ...
  | { status: "partial"; partialReason: XCodeDataSizePartialReason }
);

export interface XCodeDataSizeScanRequest {
  rootPath: string;
  maxDurationMs: number;
  maxFiles: number;
}

interface XCodeDataSizeScanOptions extends XCodeDataSizeScanRequest {
  signal?: AbortSignal;
 ...
 */
export async function scanXCodeDataDirectory(
  options: XCodeDataSizeScanOptions,
  ...
  let terminalPartialReason: Exclude<XCodeDataSizePartialReason, "io_error"> | null = null;
...
export function isXCodeDataSizeScanResult(value: unknown): value is XCodeDataSizeScanResult {
  if (typeof value !== "object" || value === null) {
    return false;
  }
  const candidate = value as Partial<XCodeDataSizeScanResult>;
  const finiteNumbers = [
    candidate.bytes,
