...
import { readXCodeStdioTapDevState } from "@zcode/services/node";
...
const HELP_ZCODE_ENDPOINT_PRODUCTION_MENU_ID = "help.xcode-endpoint.production";
...
export function updateXCodeStdioTapDevMenuState() {
  const menu = Menu.getApplicationMenu();
  const item = menu?.getMenuItemById(HELP_TOGGLE_ZCODE_STDIO_TAP_MENU_ID);
  if (!item) {
    return;
  }
  const state = readXCodeStdioTapDevState();
...
function buildApplicationMenuTemplate(options: {
  ...
  const stdioTapState = readXCodeStdioTapDevState();
    ...
    },
    {
      ...
      submenu: [
          ...
          ? [
              {
                ...
                label: getLabel(desktopMenuMessageIds.helpToggleXCodeStdioTap),
                type: "checkbox" as const,
                checked: stdioTapState.enabled,
                click: () =>
                  void options.executeDesktopCommand(DesktopCommandIds.ToggleXCodeStdioTapDevProxy),
              },
              { type: "separator" as const },
            ]
          : []),
        ...(ZCODE_ENV === "test"
          ? [
              {
                label: getLabel(desktopMenuMessageIds.helpXCodeEndpoint),
                submenu: [
                  {
                    id: HELP_ZCODE_ENDPOINT_PRODUCTION_MENU_ID,
                    label: getLabel(desktopMenuMessageIds.helpXCodeEndpointProduction),
                    type: "radio" as const,
                    checked: (options.zcodeEndpointSelection ?? "production") === "production",
                    click: () =>
                      void options.executeDesktopCommand(
                        DesktopCommandIds.SetXCodeEndpointProduction,
                      ),
                  },
                  { type: "separator" as const },
                  {
                    label: getLabel(desktopMenuMessageIds.helpXCodeEndpointCustom),
                    click: () =>
                      void options.executeDesktopCommand(DesktopCommandIds.SetXCodeEndpointCustom),
                  },
                  {
                    label: getLabel(desktopMenuMessageIds.helpXCodeEndpointReset),
                    click: () =>
                      void options.executeDesktopCommand(DesktopCommandIds.ResetXCodeEndpoint),
                  },
                ],
              },
              { type: "separator" as const },
