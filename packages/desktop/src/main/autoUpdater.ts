...
function applyManifestUpdateProvider(options: InitAutoUpdaterOptions): void {
  ...
  autoUpdater.setFeedURL({
    ...
    resolveEndpointOrigin:
      options.resolveEndpointOrigin ?? (() => resolveRuntimeXCodeEndpointOrigin(process.env)),
...
export function refreshAutoUpdaterReleaseChannel(
  ...
  // XCode fork：更新检测已禁用（initAutoUpdater 总闸），通道刷新不再触发 checkForUpdates。
  if (autoUpdaterDisabledForProductFlavor) {
    logger.info(`[auto-update] skip ${reason}: updater disabled in this fork`);
    return;
  }
...
export async function initAutoUpdater(options: InitAutoUpdaterOptions = {}): Promise<void> {
  // XCode fork：禁用更新检测总闸——启动检查、周期轮询、手动菜单检查与强更门
  // 全部依赖本初始化链路或本模块标志位，这里强制走 disabled 分支一次性关停。
  const XCODE_FORK_AUTO_UPDATES_DISABLED = true;
  if (XCODE_FORK_AUTO_UPDATES_DISABLED || options.enabled === false) {
