...
import type { XCodeGroupedTaskView, XCodeTaskGroupColor } from "@zcode/services";
import { OFF_PEAK_DEFAULT_GROUP_ID, type XCodeTaskMeta } from "@zcode/shared";
...
function getGroupedTaskViewSignature(view: XCodeGroupedTaskView): string {
  return view.nodes
    .map((node) =>
      node.type === "task"
        ? `t:${taskKey(node.task)}`
...
export function WorkspaceGroupedTasksSection({
  ...
  const removeTaskState = useXCodeSessionStore((state) => state.removeTaskState);
  const upsertOptimisticTaskListItem = useXCodeSessionStore(
    (state) => state.upsertOptimisticTaskListItem,
  );
  const setTaskUnreadIndicator = useXCodeSessionStore((state) => state.setTaskUnreadIndicator);
  const groupedDraftTask = useXCodeSessionStore(
    (state) =>
      selectWorkspaceXCodeState(state, activeWorkspacePath, activeWorkspaceIdentity)
        .groupedDraftTask,
  );
  const groupedDraftFocusVersion = useXCodeSessionStore(
    (state) =>
      selectWorkspaceXCodeState(state, activeWorkspacePath, activeWorkspaceIdentity)
        .draftFocusVersion,
  );
  const clearGroupedDraftTask = useXCodeSessionStore((state) => state.clearGroupedDraftTask);
  const {
  ...
  const dragOriginViewRef = useRef<XCodeGroupedTaskView | null>(null);
  const dragPreviewViewRef = useRef<XCodeGroupedTaskView | null>(null);
  ...
  const handleStartRenameTask = useCallback((task: XCodeTaskMeta) => {
    setRenamingTaskKey(taskKey(task));
    setRenameDraft(task.title ?? "");
  }, []);

  const handleMoveTaskToGroup = useCallback(
    (task: XCodeTaskMeta, groupId: string | null) => {
  ...
  const handleUpdateGroupColor = useCallback(
    (groupId: string, color: XCodeTaskGroupColor) => {
