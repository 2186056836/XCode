...
import type { XCodeSavedWorkflowRun, XCodeSavedWorkflowRunStatus } from "@zcode/shared";
...
): Map<string, XCodeSavedWorkflowRun> {
  const byName = new Map<string, XCodeSavedWorkflowRun>();
...
export function savedWorkflowRunBadgeKind(
  status: XCodeSavedWorkflowRunStatus | "errored" | "stopped" | "failed" | "cancelled" | undefined,
...
export function savedWorkflowRunDurationMs(run: XCodeSavedWorkflowRun, now: number): number {
