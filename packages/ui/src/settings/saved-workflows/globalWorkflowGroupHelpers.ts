...
// 也便于单测这些无 React 的判定。
import { resolveWorkspaceKey, type XCodeSavedWorkflowRun } from "@zcode/shared";
import type { XCodeAgentSavedWorkflowTarget } from "@zcode/services";
...
export const GLOBAL_SAVED_WORKFLOW_TARGET: XCodeAgentSavedWorkflowTarget = { scope: "global" };
...
export function buildGlobalRunProjectResolver(
  localProjects: readonly AutomationWorkspaceOption[],
): (run: XCodeSavedWorkflowRun) => SavedWorkflowRunProject | null {
  return (run) => {
