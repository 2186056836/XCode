...
import type { ModelConnectivityResult, UpstreamModelListResult } from "@zcode/shared";
import {
  isApiKeyAccess,
...
import {
  DEFAULT_PERSONAL_PROVIDER_API_FORMAT,
...
export function InlineEditableProviderCard({
  ...
  /** XCode fork：从上游拉取模型列表批量添加；只读端点的预设供应商不提供。 */
  ...
  /** XCode fork：隐藏 API Key 区（免费供应商凭据内置，不对用户显示）。 */
  hideApiKeySection?: boolean;
  /** XCode fork：只读端点下仍放开上游批量导入（免费供应商复用自定义供应商模型页）。 */
  upstreamImportEnabled?: boolean;
  ...
  const [apiFormat, setApiFormat] = useState<ProviderApiType>(
    provider.config.api?.type ?? DEFAULT_PERSONAL_PROVIDER_API_FORMAT,
  );
  ...
  const draftRef = useRef<ProviderDraftValues>({
    ...
    apiFormat: provider.config.api?.type ?? DEFAULT_PERSONAL_PROVIDER_API_FORMAT,
    baseUrlValue: provider.config.api?.baseUrl ?? "",
    apiKeyValue: getProviderFormApiKey(provider),
  });

  useEffect(() => {
    const resolvedApiFormat = provider.config.api?.type ?? DEFAULT_PERSONAL_PROVIDER_API_FORMAT;
  ...
  // XCode fork：上游模型批量导入。刻意不走 runSaveOperation 的单模型三态通知——
  // 整批只发一条 pending，串行保存完成后用一条汇总反馈收尾（成功/含失败），
  // 避免导入 N 个模型弹 N 组 toast。单个失败不中断整批，失败明细回传给对话框保留重试。
  const handleImportUpstreamModels = useCallback(
    async (
      modelIds: readonly string[],
      config: ModelConfigObject,
    ): Promise<{ added: number; failed: number; failedModelIds: string[] }> => {
      if (!onAddPersonalModel) throw new Error("当前设置入口未装配 Personal Model 添加能力");
      selfSaveRequestedRef.current = true;
      const feedbackKey = `models-import:${provider.providerId}`;
      showFeedback({
        key: feedbackKey,
        message: intl.formatMessage({ id: "settings.modelProvider.importModels.importing" }),
        state: "pending",
        durationMs: 0,
      });
      let added = 0;
      ...
      for (const rawModelId of modelIds) {
        ...
        if (!modelId) continue;
        try {
          // addPersonalModel 是 Host 配置写入事务，串行执行避免并发写冲突；
          // useRecommendedConfig=false + 弹窗统一配置——勾选的所有模型共用同一套 personalConfig。
          await onAddPersonalModel(
            provider.providerId,
            modelId,
          ...
          logger.warn("[ModelProviderSection] 批量导入模型添加失败", {
            providerId: provider.providerId,
            modelId,
            error,
          });
      ...
      const failed = failedModelIds.length;
      showFeedback({
        key: feedbackKey,
        message: intl.formatMessage(
          { id: "settings.modelProvider.importModels.result" },
          { added, failed },
        ),
      ...
      return { added, failed, failedModelIds };
    },
    [intl, onAddPersonalModel, provider.providerId, showFeedback],
  );
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="space-y-3">
        ...
        {isApiKeyProvider && !hideApiKeySection ? (
          <ProviderApiKeySection
        ...
        <ProviderModelsSection
          ...
          onFetchUpstreamModels={
            upstreamImportEnabled || !readOnlyEndpoints ? onFetchUpstreamModels : undefined
          }
          onImportUpstreamModels={
            (upstreamImportEnabled || !readOnlyEndpoints) && onAddPersonalModel
              ? handleImportUpstreamModels
              : undefined
          }
