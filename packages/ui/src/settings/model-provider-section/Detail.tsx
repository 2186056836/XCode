...
export function ModelProviderSectionDetail({
  ...
  /** XCode fork：从供应商上游拉取模型列表，用于批量添加。 */
  ...
  // XCode fork：免费供应商详情——复用自定义供应商的模型配置页（增删/批量导入），
  // 但连接信息只读、API Key 区隐藏（凭据内置，不对用户显示）。
  if (selectedNavItem.type === "free") {
    const freeProvider = selectedNavItem.provider;
    return (
      <InlineEditableProviderCard
        ...
        onReorderModelIds={
          onReorderProviderModels
            ? (modelIds) => onReorderProviderModels(freeProvider.providerId, modelIds)
            : undefined
        }
        onTestModel={onTestModel}
        readOnlyEndpoints
      ...
      return (
        <ProviderFamilyDetailShell header={codingPlanFamilyHeader}>
          <InlineEditableProviderCard
            ...
            onSave={onSave}
            {...modelEditingProps}
            // Coding Plan 端点由账号 Overlay 管理，不提供个人上游模型拉取入口。
            onFetchUpstreamModels={undefined}
            onReorderModelIds={
              onReorderProviderModels
