...
interface OffPeakEditViewProps {
  ...
  onDeleteHistory?: (task: XCodeOffPeakTask) => void;
...
export function OffPeakEditView({
  ...
  const thoughtLevelOption = useMemo<XCodeConfigOption | null>(
    () =>
    ...
    >
      ...
      {tab === "history" ? (
        <OffPeakHistoryTab
          ...
          {...(onOpenSession
            ? {
                onOpenSession: (task: XCodeOffPeakTask) =>
                  task.sessionId
                    ? onOpenSession({
