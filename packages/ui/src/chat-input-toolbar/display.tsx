...
function isConfigSelectValueInOptions(option: XCodeConfigOption, value: string): boolean {
  return option.options?.some((entry) => String(entry.value) === value) ?? false;
}

function getConfigSelectTriggerTestId(option: XCodeConfigOption): string | undefined {
...
export function ChatApiRetryStatus({
  ...
  intl: ReturnType<typeof useXCodeIntl>["intl"];
  locale: string;
}) {
  const retryLabel = useMemo(() => {
    ...
    // 当前 XCode Agent 只会推送某一刻的 retryDelayMs 快照，不会每秒递减。
...
export function getModeOptionDisplayLabel(
  ...
  entry: Pick<XCodeConfigSelectValue, "name" | "value">,
): string {
...
function getModeOptionLabelMessageId(
  provider: XCodeProvider | undefined,
  entry: Pick<XCodeConfigSelectValue, "value">,
): string | null {
  if (!provider) {
    return null;
  }
