...
import type { XCodeOffPeakTaskStatus } from "@zcode/shared";
...
function notifiableStatus(status: XCodeOffPeakTaskStatus): "completed" | "failed" | null {
...
export function useOffPeakTaskNotifications(params: {
  ...
  const seenStatusRef = useRef(new Map<string, XCodeOffPeakTaskStatus>());
