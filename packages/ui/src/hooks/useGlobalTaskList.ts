...
function buildWorkspaceScopes(workspaceTabs: WorkspaceTabState[]): XCodeTaskListWorkspaceScope[] {
  const scopes = new Map<string, XCodeTaskListWorkspaceScope>();
...
export function useGlobalTaskList(params: {
  ...
  const taskListVersionSignature = useXCodeSessionStore((state) =>
    JSON.stringify(
      params.workspaceTabs
        .map((tab) => {
          const workspace = selectWorkspaceXCodeState(
            state,
            tab.workspacePath,
