...
function resolveSkillSourceType(skillPath: string): SkillSourceType {
  ...
  if (normalized.includes("/.xcode/skills/")) {
    return "glm";
  }
  if (normalized.includes("/.xcode/cli/plugins/cache/")) {
    return "glm";
  }
  return "unknown";
}
...
function isXcodeSkill(skill: { id?: string; path: string; scope?: string }): boolean {
  return (
    // plugin skill 的真实路径在 CLI plugin cache 下，不在 `.xcode/skills`。
...
export function filterSkillsForProvider<T extends { path: string; id?: string; scope?: string }>(
  skills: T[],
  _legacyProvider: XCodeProvider,
): T[] {
  return skills.filter(isXcodeSkill);
}
