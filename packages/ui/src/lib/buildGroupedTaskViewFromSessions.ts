...
// sessions-index 只补实时 activity/detail，最终拼出旧 XCodeGroupedTaskView 同形结果。
...
function taskKeyOf(task: XCodeTaskMeta): string {
...
function taskOrderMapKey(task: XCodeTaskMeta): string {
...
function nodeMapKey(node: XCodeGroupedTaskViewNode): string {
...
function sortGroupTasks(
  ...
  membersByTaskKey: Map<string, XCodeGroupedTaskViewStructureMember>,
): XCodeTaskListItem[] {
  const resolvedOrderByTaskKey = new Map<string, number>();
  let maxOrder = 0;
  const missing: Array<{ task: XCodeTaskListItem; addedAt: number; key: string }> = [];
...
export function buildGroupedTaskViewFromSessions(
  ...
  const activeTaskByKey = new Map<string, XCodeTaskListItem>();
  ...
  const membersByGroupId = new Map<string, XCodeGroupedTaskViewStructureMember[]>();
  ...
  const nodes: XCodeGroupedTaskViewNode[] = structure.groups.map((group) => {
