/* oxlint-disable eslint(max-lines) -- XCode session 到当前聊天 projection 的迁移桥需要同时保持 snapshot 和 event 映射一致。 */
import {
...
] as const satisfies readonly XCodeTaskModeInfo[];
...
export function parseModelPickerValue(value: string): ModelSelection {
  ...
  if (customModel?.providerId && customModel.modelName) {
    ...
    // XCode Protocol 必须收到严格的 providerId/modelId 结构。
    return {
...
export function zcodeSessionSettingsToConfigOptions(
  ...
  const configOptions: XCodeConfigOption[] = [
    {
      id: MODEL_CONFIG_ID,
      name: "Model",
      category: "model",
    ...
    },
    {
      ...
      category: "mode",
      type: "select",
      currentValue: normalizeAvailableXCodeMode(settings.mode.current),
      options: getXCodeAgentModeSelectOptions(),
    },
  ];
  ...
  return [
    {
      ...
      category: "mode",
      type: "select",
      currentValue: normalizeAvailableXCodeMode(mode),
      options: getXCodeAgentModeSelectOptions(),
    },
  ];
}
...
function resolveSettingsThoughtLevelCurrentValue(
  thoughtLevel: XCodeSessionSettingsState["thoughtLevel"],
): string | undefined {
  ...
  const defaultThoughtLevel =
    thoughtLevel.defaultLevel && thoughtLevelValues.has(thoughtLevel.defaultLevel)
      ...
      : undefined;
  // XCode Protocol 的 defaultLevel 是模型事实，current 为空时表示用户尚未显式修改。
...
export function zcodeSessionSnapshotToTaskMeta(snapshot: XCodeSessionStateSnapshot): XCodeTaskMeta {
  return {
...
function normalizeAvailableXCodeMode(mode: XCodeSessionMode): string {
  return ZCODE_AGENT_MODE_ID_SET.has(mode) ? mode : "build";
}

function getXCodeAgentModeSelectOptions(): NonNullable<XCodeConfigOption["options"]> {
