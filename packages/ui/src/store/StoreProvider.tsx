...
import { createXCodeStore, type XCodeStore, type XCodeState } from "./index.js";

// 导出 Context 供测试直接注入已构造的 store 实例（如跨窗口广播抑制用例）。
const StoreContext = createContext<XCodeStore | null>(null);
...
export function StoreProvider({
  ...
  const storeRef = useRef<XCodeStore | null>(null);
  if (!storeRef.current) {
    storeRef.current = createXCodeStore(broadcastService, {
 ...
 */
export function useXCodeStore<T>(selector: (state: XCodeState) => T): T {
  const store = useContext(StoreContext);
  if (!store) {
    throw new Error("useXCodeStore 必须在 StoreProvider 内使用");
  }
  return useStore(store, selector);
}

/**
 * 带默认值的容错版 useXCodeStore（store 耦合剥离配套）。
 *
 ...
 */
export function useXCodeStoreWithDefault<T>(
  selector: (state: XCodeState) => T,
  defaultValue: T,
): T {
  const store = useContext(StoreContext);
