...
function buildXCodeHookLocation(
...
function hookFromConfig(config: HookConfig, workspacePath: string): Hook {
  return {
    ...
    location: buildXCodeHookLocation(workspacePath, config.storageLevel),
  };
}
...
export const useHooksStore = create<HooksStoreState>((set, get) => ({
  ...
  importHook: async (id, hooksService) => {
    ...
    const imported: Hook = {
      ...
      id: `hook-${crypto.randomUUID()}`,
      enabled: true,
      location: buildXCodeHookLocation(workspacePath, source.location?.scope ?? "user"),
    };
