...
export const ConversationAssistantTextActions = memo(function ConversationAssistantTextActions({
  ...
  onRetry?: (target: ConversationRowTarget) => void;
  onFeedbackChange?: AssistantFeedbackHandler;
  /** XCode fork：原地编辑助手消息文本（保存后作为真实上下文进入后续请求）。 */
  onEditAssistant?: (target: ConversationRowTarget, newText: string) => Promise<unknown> | void;
  ...
  const editAssistantLabel = intl.formatMessage({ id: "chat.message.editAssistant" });
  ...
  // XCode fork：助手消息原地编辑——编辑区替换动作栏；保存 dispatch 命令，
  // 文本刷新等服务端 row.upserted 投影（失败保持编辑态供重试）。
  if (editingAssistant) {
    return (
      <div className={cn("flex flex-col gap-2", className)}>
        <textarea
          ...
          className="w-full resize-y rounded-lg border border-input-border bg-input px-3 py-2 font-mono text-ui-sm text-foreground outline-none focus-visible:ring-2 focus-visible:ring-input-border-focused/30"
        />
        <div className="flex justify-end gap-2">
          <Button
            ...
            size="sm"
            disabled={editSaving}
            onClick={() => setEditingAssistant(false)}
          >
          ...
          <Button
            ...
            onClick={() => {
              if (!entityId || !onEditAssistant) return;
              setEditSaving(true);
              void Promise.resolve(onEditAssistant({ rowId, entityId }, editText)).then(
  ...
  return (
    <MessageActions className={cn(className)}>
      ...
      {canEditAssistant && onEditAssistant && entityId ? (
        <MessageAction
          aria-label={editAssistantLabel}
          label={editAssistantLabel}
          tooltip={resolveTooltip(editAssistantLabel)}
          onClick={() => {
            setEditText(text);
