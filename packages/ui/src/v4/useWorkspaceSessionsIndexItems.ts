// 把若干 workspace scope 的 sessions-index 会话聚合成 XCodeTaskMeta[]（响应式），
...
/**
 * 订阅给定 workspace scope 的 sessions-index，聚合出 XCodeTaskMeta[]（running 置顶，其余按 updatedAt 降序）。
...
export function useWorkspaceSessionsIndexItems(
  ...
  // 从各 store 聚合会话 → XCodeTaskMeta + hydration 状态（tick 驱动重算，引用稳定）。
  const previousItemsRef = useRef<XCodeTaskMeta[]>([]);
  const previousHydratingRef = useRef<string[]>([]);
  return useMemo(() => {
    const metas: XCodeTaskMeta[] = [];
    ...
    metas.sort((a, b) => compareXCodeTaskListItems(a, b, "updated"));
