import { ZCODE_AGENT_PROVIDER, type PlanIdentitySnapshot, type XCodeProvider } from "@zcode/shared";
...
/** V4 config.provider 是实际模型 provider id；agentProvider 表示 XCode 运行时。 */
export function buildV4ConversationPromptTelemetryExtraDetail(params: {
  agentProvider?: XCodeProvider;
  configProvider?: string | null;
  modelName?: string | null;
  askMode?: string | null;
  ...
  return {
    ...
    // agent 表示 XCode 运行时，不能用模型 provider id 替代。
    agent: agentProvider,
  };
}
