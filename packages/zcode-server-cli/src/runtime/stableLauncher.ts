...
function stablePosixLauncher(bootstrap?: StableLauncherBootstrap): string {
  const fallback = bootstrap
    ...
    : 'echo "No current XCode Server release" >&2; exit 1';
  return `#!/bin/sh
...
if [ -n "$RELEASE_DIR" ]; then
  case "$RELEASE_DIR" in
    "$ROOT/releases"/*) ;;
    *) echo "Invalid current XCode Server release" >&2; exit 1 ;;
  esac
...
function stableWindowsLauncher(bootstrap?: StableLauncherBootstrap): string {
  const fallback = bootstrap
    ...
    : "echo No current XCode Server release 1>&2\r\nexit /b 1\r\n";
  return `@echo off\r
set "ROOT=%~dp0.."\r
...
if errorlevel 1 (\r
  echo Invalid current XCode Server release 1>&2\r
  exit /b 1\r
)\r
...
export async function writeStableLauncher(
  ...
  const launcherPath = join(layout.stableBinDir, platform === "win32" ? "xcode.cmd" : "xcode");
