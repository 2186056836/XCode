...
export type WorkspaceHookConfigFileKind = "zcode.json" | ".xcode/config.json" | "explicit";
...
function buildWorkspaceHookCandidatePaths(directories: readonly string[]): string[] {
  return directories.flatMap((directory) => [
    ...
    join(directory, ".xcode", "config.json"),
  ]);
}
...
export function createWorkspaceHookSourceInput(input: {
  ...
  return {
    canonicalPath,
    baseDir: basename(configDirectory) === ".xcode" ? dirname(configDirectory) : configDirectory,
    ...
    editable:
      !explicitProjectConfig &&
      canonicalPath === resolve(input.workingDirectory, ".xcode", "config.json"),
    hooks: input.hooks,
  };
}
