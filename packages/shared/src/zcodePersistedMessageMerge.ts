import type { XCodePersistedMessage, XCodePersistedMessagePart } from "./zcode-task-types-core.js";

// XCode runtime 把同一个 user turn 里**每一轮 LLM 调用**落成独立的 assistant 消息（各带 time.created/completed）。
...
export function coalesceConsecutiveXCodeAssistants(
  messages: readonly XCodePersistedMessage[],
): XCodePersistedMessage[] {
  const merged: XCodePersistedMessage[] = [];
...
function mergeAssistantPair(
  first: XCodePersistedMessage,
  next: XCodePersistedMessage,
): XCodePersistedMessage {
  ...
  const shiftedNextParts: XCodePersistedMessagePart[] = (next.parts ?? []).map((part) =>
    part.type === "tool-call"
