...
export const commandPayloadSchemas = {
  ...
  // XCode fork：原地编辑已完成助手消息的文本；保存后作为真实上下文进入后续请求。
  // 不截断后续消息、不重跑（与 editUserQuery 的 rewind+重发语义不同）。
  editAssistantMessage: z.object({
    target: conversationRowTargetSchema,
...
export type CommandPayloadMap = {
  [T in CommandType]: T extends "sendText"
    ? z.infer<(typeof commandPayloadSchemas)[T]> &
        import("../zcode-task-types-core.js").XCodeBackgroundTurnAttribution
    : z.infer<(typeof commandPayloadSchemas)[T]>;
};
