/* XCode 官方 Server MCP 鉴权的共享常量与类型。
...
export interface IsOfficialMcpOriginTrustedInput {
   ...
   */
  pluginId: string;
  /** 当前 XCode API origin，由调用方按各自口径解析后传入。 */
  zcodeApiOrigin: string | undefined;
}

/**
 * 校验官方 MCP 的凭据目标：要求 HTTPS origin 与运行时 XCode API origin 相等，
...
export interface CreateOfficialMcpTrustedOriginRegistryOptions {
  ...
  devTrustedOriginsRaw?: string | undefined;
  /** 当前 XCode API origin 的解析器；两侧必须用等价口径，否则会一侧放行一侧拒绝。 */
  resolveXCodeApiOrigin: () => string | undefined | Promise<string | undefined>;
}
...
export function createOfficialMcpTrustedOriginRegistry(
  ...
  return {
    async isTrusted({ origin, pluginId }) {
      ...
      try {
        zcodeApiOrigin = await options.resolveXCodeApiOrigin();
      } catch {
